...
sim_list <- Setup_Sim_Dim(n_sims = 20, # number of simulations to conduct
                          n_yrs = 100, # number of years
...
sim_list <- Setup_Sim_FishMort(sim_list = sim_list, # simulation list defined from above
                               ...
                               Fmort_fct = matrix(c(15, 15), nrow = sim_list$n_regions, ncol = sim_list$n_fish_fleets), # factor to multiply fmort_start by to reach apicial F
...
sim_list <- Setup_Sim_Rec(
  ...
  base_r0 = c(50, 50), # base R0 values (or mean)
...
Because movement dynamics can be incredibly complex, we have designed it such that users supply their own defined movement "matrix". This matrix then needs to be input into the `sim_list` object for use. Here, we are defining uniform movement for simplicity. 
```{r, eval = FALSE}
ref <- 1
