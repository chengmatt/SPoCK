...
The following tables describes all elements contained within `input_list$data`, which is generated using the `SPoCK::Setup_x` functions. Note that when `n_sexes > 1`, the first dimension will always be females and the second dimension will always be males. For further details on how arguments for `SPoCK::Setup_x` functions should be defined, users should refer to the function documentation.
