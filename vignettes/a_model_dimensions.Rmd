...
`censor_regional_catch` | Whether or not to censor regional catch 0 == don't censor, 1 == censor (can be used in years where catch is aggregated across regions and not spatially explicit)
`ub_catch_censor` | Upper bound of catch censoring to use, dimensioned by n_regions, n_years, and n_fish_fleets
`lb_catch_censor` | Lower bound of catch censoring to use, dimensioned by n_regions, n_years, and n_fish_fleets
`catch_censor_sd` | Standard deviation to use for catch censoring. Smaller values indicate that predictions should be within the defined bounds, larger values allow for more deviations and penalize predictions outside of the bounds less
...
`FishAgeComps_Type` |	Matrix dimensioned by n_years, n_fish_fleets specifying how age composition data should be structured and fit to. 0: Aggregated across sexes and regions, 1: Split by sexes and regions, 2: Joint by sex but split by region, 3: Joint by sex and region. More options and details can be found in `Get_Comp_Likelihoods.R`
`FishLenComps_Type` |	Matrix dimensioned by n_years, n_fish_fleets specifying how length composition data should be structured and fit to. 0: Aggregated across sexes and regions, 1: Split by sexes and regions, 2: Joint by sex but split by region, 3: Joint by sex and region. More options and details can be found in `Get_Comp_Likelihoods.R`
...
`SrvAgeComps_Type` |	Matrix dimensioned by n_years, n_srv_fleets specifying how age composition data should be structured and fit to. 0: Aggregated across sexes and regions, 1: Split by sexes and regions, 2: Joint by sex but split by region, 3: Joint by sex and region. More options and details can be found in `Get_Comp_Likelihoods.R`
`SrvLenComps_Type` |	Matrix dimensioned by n_years, n_srv_fleets specifying how length composition data should be structured and fit to. 0: Aggregated across sexes and regions, 1: Split by sexes and regions, 2: Joint by sex but split by region, 3: Joint by sex and region. More options and details can be found in `Get_Comp_Likelihoods.R`
