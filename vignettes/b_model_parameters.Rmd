...
The following table describes all elements contained within `input_list$pars`, which are generated using the `SPoCK::Setup_x` functions. Note that when `n_sexes > 1`, the first dimension will always be females and the second dimension will always be males.
...
| `logit_move_devs`       | `n_regions`, `n_regions – 1`, `n_years`, `n_ages`    | Movement               | Logit movement deviations                                               |
| `move_pe_pars`          | `n_regions`, `n_ages`                                | Movement               | Log process error parameters                                            |
