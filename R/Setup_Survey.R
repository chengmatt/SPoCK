...
Setup_Mod_SrvIdx_and_Comps <- function(input_list,
  ...
  for(f in 1:ncol(srv_idx_type_vals)) {
    ...
    message(paste("Survey Index", "for survey fleet", f, "specified as:" , srv_idx_type[f]))
  } # end f loop
  ...
  for(f in 1:input_list$data$n_srv_fleets) {
    ...
    message(paste("Survey Age Composition Likelihoods", "for survey fleet", f, "specified as:" , SrvAgeComps_LikeType[f]))
  } # end f loop
  ...
  for(f in 1:input_list$data$n_srv_fleets) {
    ...
    message(paste("Survey Length Composition Likelihoods", "for survey fleet", f, "specified as:" , SrvLenComps_LikeType[f]))
  } # end f loop
  ...
  if(is.null(ISS_SrvAgeComps)) {
    message("No ISS is specified for SrvAgeComps. ISS weighting is calculated by summing up values from ObsSrvAgeComps each year")
  ...
  if(is.null(ISS_SrvLenComps)) {
    message("No ISS is specified for SrvLenComps. ISS weighting is calculated by summing up values from ObsSrvLenComps each year")
...
Setup_Mod_Srvsel_and_Q <- function(input_list,
  ...
  message("Survey Catchability priors are: ", ifelse(Use_srv_q_prior == 0, "Not Used", "Used"))
  ...
  for(i in 1:length(cont_tv_srv_sel)) {
    ...
    message("Continuous survey time-varying selectivity specified as: ", cont_tv_type, " for survey fleet ", fleet)
  }
  ...
  for(f in 1:input_list$data$n_srv_fleets) message(paste("Survey Selectivity Time Blocks for survey", f, "is specified at:", length(unique(srv_sel_blocks_arr[,,f]))))
  ...
  for(i in 1:length(srv_sel_model)) {
    ...
    message("Survey selectivity functional form specified as:", sel_type, " for survey fleet ", f)
  } # end i loop
  ...
  for(f in 1:input_list$data$n_srv_fleets) message(paste("Survey Catchability Time Blocks for fishery", f, "is specified at:", length(unique(srv_q_blocks_arr[,,f]))))
  ...
  for(f in 1:input_list$data$n_srv_fleets) {
    ...
    message("srv_fixed_sel_pars_spec is specified as: ", srv_fixed_sel_pars_spec[f], " for survey fleet ", f)
  ...
  for(f in 1:input_list$data$n_srv_fleets) {
    ...
    message("srv_q_spec is specified as: ", srv_q_spec[f], " for survey fleet ", f)
  } # end f loop
  ...
  for(f in 1:input_list$data$n_srv_fleets) {
        ...
        for(s in 1:input_list$data$n_sexes) {
          ...
          if(input_list$data$cont_tv_srv_sel[r,f] %in% c(3,4,5)) {
            ...
            if(!is.null(corr_opt_semipar)) {
              ...
              message("corr_opt_semipar is specified as: ", corr_opt_semipar[f], "for survey fleet", f)
    ...
    if(!is.null(srvsel_pe_pars_spec)) message("srvsel_pe_pars_spec is specified as: ", srvsel_pe_pars_spec[f], " for survey fleet ", f)
  ...
  for(r in 1:input_list$data$n_regions) {
    for(f in 1:input_list$data$n_srv_fleets) {
      ...
      if(!is.null(srv_sel_devs_spec)) message("srv_sel_devs_spec is specified as: ", srv_sel_devs_spec[f], " for survey fleet ", f, " and region ", r)
