        ...
        for(r in 1:n_regions) {
          for(s in 1:n_sexes) {
            ...
            if(comp_strc == 0) {
              ...
              if(comp_fish_like[f] == 0) Obs_FishAgeComps[y,r,,s,f,sim] <- array(stats::rmultinom(1, ISS_FishAge, tmp_FishAgeComps_Prob / sum(tmp_FishAgeComps_Prob)), dim = c(dim(CAA[y,r,,s,f,sim]))) # simulate multinomial probabilities
              if(comp_fish_like[f] == 1) Obs_FishAgeComps[y,r,,s,f,sim] <- array(rdirM(1, ISS_FishAge, (FishAgeTheta * ISS_FishAge) * tmp_FishAgeComps_Prob / sum(tmp_FishAgeComps_Prob)), dim = dim(tmp_FishAgeComps_Prob)) # Simulate dirichlet multinomial samples
          ...
          if(comp_strc == 1) {
            ...
            if(comp_fish_like[f] == 0) Obs_FishAgeComps[y,r,,,f,sim] <- array(stats::rmultinom(1, ISS_FishAge, tmp_FishAgeComps_Prob / sum(tmp_FishAgeComps_Prob)), dim = c(dim(CAA[y,r,,,f,sim, drop = FALSE]))) # simulate multinomial probabilities
            if(comp_fish_like[f] == 1) Obs_FishAgeComps[y,r,,,f,sim] <- array(rdirM(1, ISS_FishAge, (FishAgeTheta *  ISS_FishAge) * tmp_FishAgeComps_Prob / sum(tmp_FishAgeComps_Prob)), dim = c(dim(CAA[y,r,,,f,sim, drop = FALSE]))) # Simulate dirichlet multinomial samples
        ...
        for(r in 1:n_regions) {
          for(s in 1:n_sexes) {
            ...
            if(comp_strc == 0) {
              ...
              if(comp_srv_like[sf] == 0) Obs_SrvAgeComps[y,r,,s,sf,sim] <- array(stats::rmultinom(1, ISS_SrvAge, tmp_SrvAgeComps_Prob / sum(tmp_SrvAgeComps_Prob)), dim = c(dim(Srv_IAA[y,r,,s,sf,sim]))) # simulate multinomial probabilities
            } # end if for "Split" approach for composition data (split by region and sex)
          ...
          if(comp_strc == 1) {
            ...
            if(comp_srv_like[sf] == 0) Obs_SrvAgeComps[y,r,,,sf,sim] <- array(stats::rmultinom(1, ISS_SrvAge, tmp_SrvAgeComps_Prob / sum(tmp_SrvAgeComps_Prob)), dim = c(dim(Srv_IAA[y,r,,,sf,sim,drop=FALSE]))) # simulate multinomial probabilities
            if(comp_srv_like[sf] == 1) Obs_SrvAgeComps[y,r,,,sf,sim] <- array(rdirM(1, ISS_SrvAge, (SrvAgeTheta * ISS_SrvAge) * tmp_SrvAgeComps_Prob / sum(tmp_SrvAgeComps_Prob)), dim = c(dim(Srv_IAA[y,r,,,sf,sim,drop=FALSE]))) # simulate multinomial probabilities
          } # end if for 'Split' approach for composition data split by region by not by sex
        } # end r loop
