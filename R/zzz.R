...
.onAttach <- function(libname, pkgname) {
  ...
  if (!requireNamespace("OSA_multivariate_dists", quietly = TRUE)) {
    packageStartupMessage("Installing OSA_multivariate_dists from GitHub...")
    tryCatch({
  ...
  if (!requireNamespace("compResidual", quietly = TRUE)) {
    packageStartupMessage("Installing compResidual from GitHub...")
    tryCatch({
  ...
  if (!requireNamespace("afscOSA", quietly = TRUE)) {
    packageStartupMessage("Installing afscOSA from GitHub...")
    tryCatch({
      install_afscOSA()
    }, error = function(e) {
